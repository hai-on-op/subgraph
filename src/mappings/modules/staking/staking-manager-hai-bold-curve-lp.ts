...
} from '../../../../generated/StakingManagerHaiBoldCurveLp/StakingManager'
import {
  HaiBoldCurveLPStakingUser,
  HaiBoldCurveLPStakingPosition,
  HaiBoldCurveLPPendingWithdrawal,
  HaiBoldCurveLPRewardClaim,
...
export function handleStakingManagerStaked(event: StakingManagerStaked): void {
    ...
    let position = new HaiBoldCurveLPStakingPosition(
  ...
  export function handleStakingManagerWithdrawalInitiated(event: StakingManagerWithdrawalInitiated): void {
    ...
    let pendingWithdrawal: HaiBoldCurveLPPendingWithdrawal
    
    if (existingPendingWithdrawalId) {
      // Load existing pending withdrawal
      pendingWithdrawal = HaiBoldCurveLPPendingWithdrawal.load(existingPendingWithdrawalId) as HaiBoldCurveLPPendingWithdrawal
      ...
      pendingWithdrawal = new HaiBoldCurveLPPendingWithdrawal(event.params._account.toHexString())
  ...
  export function handleStakingManagerWithdrawalCancelled(event: StakingManagerWithdrawalCancelled): void {
    let user = getOrCreateUser(event.params._account)
    let pendingWithdrawal = HaiBoldCurveLPPendingWithdrawal.load(event.params._account.toHexString())
    let position = new HaiBoldCurveLPStakingPosition(
      event.params._account.toHexString() + event.block.timestamp.toString()
    )
  ...
  export function handleStakingManagerRewardPaid(event: StakingManagerRewardPaid): void {
    let user = getOrCreateUser(event.params._account)
    let rewardClaim = new HaiBoldCurveLPRewardClaim(
      event.params._account.toHexString() + event.block.timestamp.toString()
    )
  ...
  // Helper functions
  function getOrCreateUser(address: Address): HaiBoldCurveLPStakingUser {
    let user = HaiBoldCurveLPStakingUser.load(address.toHexString())
    
    if (!user) {
      user = new HaiBoldCurveLPStakingUser(address.toHexString())
  ...
  function getOrCreateStakingStatistics(): HaiBoldCurveLPStakingStatistic {
    let stats = HaiBoldCurveLPStakingStatistic.load("singleton")
    
    if (!stats) {
      stats = new HaiBoldCurveLPStakingStatistic("singleton")
