...
} from '../../../../generated/StakingManagerHaiVeloVeloLp/StakingManager'
import {
  HaiVeloVeloLPStakingUser,
  HaiVeloVeloLPStakingPosition,
  HaiVeloVeloLPPendingWithdrawal,
  HaiVeloVeloLPRewardClaim,
...
export function handleStakingManagerStaked(event: StakingManagerStaked): void {
    ...
    let position = new HaiVeloVeloLPStakingPosition(
  ...
  export function handleStakingManagerWithdrawalInitiated(event: StakingManagerWithdrawalInitiated): void {
    ...
    let pendingWithdrawal: HaiVeloVeloLPPendingWithdrawal
    
    if (existingPendingWithdrawalId) {
      // Load existing pending withdrawal
      pendingWithdrawal = HaiVeloVeloLPPendingWithdrawal.load(existingPendingWithdrawalId) as HaiVeloVeloLPPendingWithdrawal
      ...
      pendingWithdrawal = new HaiVeloVeloLPPendingWithdrawal(event.params._account.toHexString())
  ...
  export function handleStakingManagerWithdrawalCancelled(event: StakingManagerWithdrawalCancelled): void {
    let user = getOrCreateUser(event.params._account)
    let pendingWithdrawal = HaiVeloVeloLPPendingWithdrawal.load(event.params._account.toHexString())
    let position = new HaiVeloVeloLPStakingPosition(
      event.params._account.toHexString() + event.block.timestamp.toString()
    )
  ...
  export function handleStakingManagerRewardPaid(event: StakingManagerRewardPaid): void {
    let user = getOrCreateUser(event.params._account)
    let rewardClaim = new HaiVeloVeloLPRewardClaim(
      event.params._account.toHexString() + event.block.timestamp.toString()
    )
  ...
  // Helper functions
  function getOrCreateUser(address: Address): HaiVeloVeloLPStakingUser {
    let user = HaiVeloVeloLPStakingUser.load(address.toHexString())
    
    if (!user) {
      user = new HaiVeloVeloLPStakingUser(address.toHexString())
  ...
  function getOrCreateStakingStatistics(): HaiVeloVeloLPStakingStatistic {
    let stats = HaiVeloVeloLPStakingStatistic.load("singleton")
    
    if (!stats) {
      stats = new HaiVeloVeloLPStakingStatistic("singleton")
